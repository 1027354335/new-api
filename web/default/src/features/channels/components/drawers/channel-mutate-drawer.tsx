...
export function ChannelMutateDrawer({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      toast.warning(
        t(
          'Warning: Base URL should not end with /v1. OSS API will handle it automatically. This may cause request failures.'
        ),
        { duration: 5000 }
      )
    }, 500)
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {t(
                            'Custom API base URL. For official channels, OSS API has built-in addresses. Only fill this for third-party proxy sites or special endpoints. Do not add /v1 or trailing slash.'
                          )}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
